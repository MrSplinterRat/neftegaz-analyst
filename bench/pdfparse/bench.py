...
def match(candidates: list[str], needle: str) -> int:
    """Индекс подходящего элемента: сперва ТОЧНОЕ совпадение, затем подстрока.

    ★ПОРЯДОК НЕСУЩИЙ, и он оплачен предельным случаем корпуса. В `Table 9a`
    имена строк повторяются на разных ярусах: блок промышленного производства
    несёт «Petroleum and coal products» и «Natural Gas-weighted manufacturing»,
    а блок выбросов ниже — просто «Petroleum» и «Natural gas». Поиск первого
    вхождения ПОДСТРОКИ брал верхнюю строку, сравнивал её числа с эталоном
    нижней и объявлял 15 ошибок из 15 — у ВСЕХ трёх настоящих кандидатов разом.

    ★Это отказ прибора, а не кандидатов, и распознать его можно было только по
    величине: три независимых читателя не ошибаются одинаково на 45 ячейках,
    выдавая при этом числа из другой части страницы. Согласие подозреваемых —
    признак того, что виноват судья.

    Подстрока остаётся запасным путём: заголовки таблиц сверяются по первым
    сорока знакам, и точного совпадения там не бывает по построению.
    """
    lowered = needle.lower().strip()
    for index, item in enumerate(candidates):
        if str(item).lower().strip() == lowered:
            return index
    for index, item in enumerate(candidates):
