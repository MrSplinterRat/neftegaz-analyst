...
class ForecastResult:
    ...
    def interpretation(self, instrument: str = "Brent") -> str:
        ...
        # ★НЕСОШЕДШАЯСЯ ПОДГОНКА — ХУДШИЙ ВИД ОТКАЗА: он молчит. Исключения
        # оптимизатор не бросает, числа выглядят как обычный ответ, и до сих пор
        # признак был доступен разработчику (в параметрах результата) и невидим
        # читателю. `params.get` возвращает None у методов, где сходимости нет
        # как понятия (сглаживание), — там строки не появится, и это верно.
        unconverged = (
            " ⚠ Подгонка модели НЕ СОШЛАСЬ: оптимизатор остановился, не найдя устойчивых "
            "коэффициентов. Числа ниже посчитаны, но опираться на них как на прогноз "
            "нельзя — сравните их с другим методом или возьмите более длинный ряд."
            if self.params.get("converged") is False
            else ""
        )
        ...
        return (
            ...
            f"остатков {self.residual_sigma:.2f} долл./барр.{flat}{unconverged}{fallback}"
        )
