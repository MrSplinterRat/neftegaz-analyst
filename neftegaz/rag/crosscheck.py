...
def _read_extractor(path: str) -> list[str] | None:
    try:
        ...
        #
        # ★attach=False — ТОТ ЖЕ ДОВОД для склейки подстрочных знаков (CO₂). Она
        # тоже наша правка поверх прочитанного, и включённой она СКРЫЛА БЫ ровно
        # тот дефект, из-за которого написана: сверка на 26 страницах видела
        # осиротевшую «2» и называла нас выбросом — и правильно делала. Пусть
        # продолжает называть, если склейка однажды перестанет срабатывать.
        return [page.text(dedupe=False, attach=False) for page in parse_pdf(path).pages]
