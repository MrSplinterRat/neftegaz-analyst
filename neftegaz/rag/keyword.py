...
class BM25Index:
    ...
    зависимость в поставке заказчику должна себя оправдывать.

    Замерено на корпусе из 9474 фрагментов: сборка 0.21 с (один раз на
    процесс), запрос 0.08 с, пик памяти процесса 371 МБ вместе с моделью
    эмбеддингов. Здесь стояла оценка «строится за секунды» — она была верна,
    но оценкой, а не числом.
    """
