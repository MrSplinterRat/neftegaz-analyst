...
class Hit:
    ...
    caveats: tuple[str, ...] = ()
    # ★Идентификатор фрагмента, выведенный из его содержания (см. chunk_id
    # выше). Едет до самого ответа, потому что по нему разговор может сослаться
    # на найденное — а восстановить его из остальных полей нельзя: в ключ
    # входят `kind` и `start`, которых у Hit нет и быть не должно.
    #
    # ⚠Ссылка живёт ровно столько, сколько живёт неизменный фрагмент: изменился
    # текст — изменился и идентификатор, и старая ссылка перестаёт находиться.
    # Это верное поведение, а не дефект: изменившийся фрагмент уже не тот, на
    # который ссылались. Ломаться оно обязано ВСЛУХ.
    chunk_id: str = ""
        ...
        for identifier in order:
            ...
            hits.append(
                Hit(
                    ...
                    # Идентификатор точки в хранилище И ЕСТЬ chunk_id: он ставится
                    # при индексации (`index` выше), а не выдаётся хранилищем.
