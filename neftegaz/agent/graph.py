...
# ── три класса полей состояния, и каждое поле обязано быть в одном из них ──
#
# ★ПРАВИЛО ИСПОЛНЯЕТСЯ МАШИНОЙ, А НЕ ПАМЯТЬЮ. «Поля-однодневки пишутся каждый
# ход» было записано словами рядом с кодом — и не сработало ТРИЖДЫ подряд, с
# одним и тем же исходом: новое поле под правило не попадало, а замечали это
# двумя ходами разговора и много позже.
#   1. `scenario_waived` — правило и родилось из него;
#   2. `reports_status` и `web_status` — отказ веба на вопросе про цену приезжал
#      оговоркой в ответ на СЛЕДУЮЩИЙ вопрос, который в веб не ходил;
#   3. `confidence_marks` и `context_used` — панель показывала бы разметку и
#      наполнение контекста ПРОШЛОГО хода как нынешние.
# Поэтому перечень стои́т здесь, рядом с полями, а тест требует, чтобы он
# накрывал их все: новое поле, не отнесённое ни к одному классу, красит приёмку
# сразу, а не через два хода разговора у пользователя.
#
# Поле-однодневка, не сброшенное явно, живёт в чекпоинтере ВЕЧНО: узел, который
# его пишет, выполняется не на всякой ветке, а читают его всегда.
ONE_TURN_FIELDS: dict[str, Any] = {
    # что спросили и куда сходили
    "route": "",
    "pending_question": "",
    "scenario_waived": False,
    # что нашли: хиты прошлого вопроса, дожившие до следующего, попали бы в
    # контекст как найденное СЕЙЧАС — то есть модель отвечала бы по чужому
    "report_hits": [],
    "web_hits": [],
    "used_reports": False,
    "used_web": False,
    "fed_chunk_ids": [],
    "stale_links": 0,
    # почему источник промолчал
    "reports_status": "",
    "web_status": "",
    # что посчитали
    "forecast_text": "",
    # что получилось и из чего собрано
    "answer": "",
    "confidence_marks": {},
    "context_used": {},
}

# Пишутся ВХОДОМ на каждом ходу (см. answer_question). Сбрасывать их в
# node_route нельзя: он выполняется после входа и затёр бы только что
# положенное.
INPUT_FIELDS: frozenset[str] = frozenset({"question", "linked_chunk_ids"})

# Живут дольше одного хода ПО ЗАМЫСЛУ: история разговора и горизонт последнего
# расчёта («а на год?» опирается ровно на него).
LASTING_FIELDS: frozenset[str] = frozenset({"history", "last_horizon_days"})


def fresh_turn_fields() -> dict[str, Any]:
    """Сброс полей-однодневок к их значениям «ничего не было».

    Копия, а не сам перечень: перечень — общий объект модуля, и отдать его
    вызывающему значило бы отдать изменяемые списки и словари, которые узлы
    графа потом дополняют.
    """
    fresh: dict[str, Any] = {}
    for key, value in ONE_TURN_FIELDS.items():
...
def node_route(state: AgentState) -> AgentState:
    ...
    #
    # ★СБРОС ИДЁТ ПО ПЕРЕЧНЮ `ONE_TURN_FIELDS`, а не по списку, набранному здесь
    # руками. Список руками — ровно то, что уже трижды отставало от состояния:
    # поле добавляли, а сброс дописать забывали, и узнавали об этом двумя ходами
    # разговора. Перечень лежит рядом с объявлением полей, и тест требует, чтобы
    # он накрывал их все.
    #
    # Копия обязательна: перечень — общий объект модуля, и возврат его самого
    # отдал бы вызывающему изменяемые списки и словари из него.
    resolved: AgentState = fresh_turn_fields()
...
def initial_state(question: str, thread: str) -> AgentState:
    """Состояние, с которого начинается ход: ровно поля класса `INPUT_FIELDS`.

    Отдельная функция, а не выражение внутри `answer_question`, потому что
    деление полей на классы проверяется тестом: «пишется входом каждый ход» —
    утверждение о ПОВЕДЕНИИ, и проверять его надо вызовом, а не чтением кода.
    """
    return {
        "question": question,
...
def answer_question(question: str, thread_id: str | None = None) -> AgentState:
    ...
    initial = initial_state(question, thread)
